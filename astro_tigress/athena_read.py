...
def hst(filename, raw=False):
    ...
    if not raw:
        if data_names[0] != "time":
            raise AthenaError(
                "Cannot remove spurious data because time column could not"
                + " be identified"
            )
...
def athdf(
    ...
    if raw:
        ...
        with h5py.File(filename, "r") as f:
            ...
            dataset_names = np.array(
                [x.decode("ascii", "replace") for x in f.attrs["DatasetNames"][:]]
            )
            dataset_sizes = f.attrs["NumVariables"][:]
            variable_names = np.array(
                [x.decode("ascii", "replace") for x in f.attrs["VariableNames"][:]]
            )
    ...
    with h5py.File(filename, "r") as f:
        ...
        if (
            level < max_level
            and not subsample
            and not fast_restrict
            and vol_func is None
        ):
            ...
            if (
                ...
                if nx1 == 1 and nx2 == 1 and (nx3 == 1 or x3_rat == 1.0):
                    ...
                    def vol_func(rm, rp, thetam, thetap, phim, phip):
                        return (
                            (rp**3 - rm**3)
                            * abs(np.cos(thetam) - np.cos(thetap))
                            * (phip - phim)
                        )
                ...
                if nx1 == 1 and nx2 == 1 and (nx3 == 1 or x3_rat == 1.0):
                    ...
                    def vol_func(rm, rp, thetam, thetap, phim, phip):
                        ...
                        cosp = np.cos(thetap)
                        return (
                            (rp**3 - rm**3) * abs(cosm - cosp)
                            + a**2 * (rp - rm) * abs(cosm**3 - cosp**3)
                        ) * (phip - phim)
            else:
        ...
        if level < max_level and (subsample or fast_restrict):
            if num_ghost > 0:
                raise AthenaError(
                    "Subsampling and fast restriction incompatible with"
                    + " ghost zones"
                )
        ...
        if not new_data:
            ...
            for q in quantities:
                if q not in var_quantities and q not in coord_quantities:
                    ...
                    error_string = (
                        'Quantity not recognized: file does not include "{0}"'
                        + " but does include {1}"
                    )
        ...
        for d, nx, face_func, center_func in zip(
            list(range(1, 4)), nx_vals, face_funcs, center_funcs
        ):
            xf = "x" + repr(d) + "f"
            xv = "x" + repr(d) + "v"
            if nx == 1:
                ...
                if xrat_root == -1.0 and face_func is None:
                    ...
                    if num_ghost > 0:
                        raise AthenaError(
                            "Ghost zones incompatible with user-defined"
                            + " coordinate spacing"
                        )
                    ...
                    if np.all(levels == level):
                        ...
                        if num_ghost > 0:
                            raise AthenaError(
                                "Cannot use ghost zones with different"
                                + " refinement levels"
                            )
                    ...
                    if num_ghost > 0:
                        raise AthenaError(
                            "Ghost zones incompatible with non-uniform"
                            + " coordinate spacing"
                        )
        ...
        if x1_min is not None and x1_min >= data["x1f"][1]:
            if x1_min >= data["x1f"][-1]:
                raise AthenaError(
                    error_string.format("x1_min", "less", data["x1f"][-1])
                )
            x1_select = True
        ...
        if x1_max is not None and x1_max <= data["x1f"][-2]:
            if x1_max <= data["x1f"][0]:
                raise AthenaError(
                    error_string.format("x1_max", "greater", data["x1f"][0])
                )
            x1_select = True
        ...
        if x2_min is not None and x2_min >= data["x2f"][1]:
            if x2_min >= data["x2f"][-1]:
                raise AthenaError(
                    error_string.format("x2_min", "less", data["x2f"][-1])
                )
            x2_select = True
        ...
        if x2_max is not None and x2_max <= data["x2f"][-2]:
            if x2_max <= data["x2f"][0]:
                raise AthenaError(
                    error_string.format("x2_max", "greater", data["x2f"][0])
                )
            x2_select = True
        ...
        if x3_min is not None and x3_min >= data["x3f"][1]:
            if x3_min >= data["x3f"][-1]:
                raise AthenaError(
                    error_string.format("x3_min", "less", data["x3f"][-1])
                )
            x3_select = True
        ...
        if x3_max is not None and x3_max <= data["x3f"][-2]:
            if x3_max <= data["x3f"][0]:
                raise AthenaError(
                    error_string.format("x3_max", "greater", data["x3f"][0])
                )
            x3_select = True
        ...
        for block_num in range(num_blocks):
            ...
            if block_level <= level:
                ...
                il_d = (
                    block_location[0] * (block_size[0] - 2 * num_ghost) * s
                    if nx1 > 1
                    else 0
                )
                jl_d = (
                    block_location[1] * (block_size[1] - 2 * num_ghost) * s
                    if nx2 > 1
                    else 0
                )
                kl_d = (
                    block_location[2] * (block_size[2] - 2 * num_ghost) * s
                    if nx3 > 1
                    else 0
                )
                ...
                # Assign values
                for q, dataset, index in zip(
                    quantities, quantity_datasets, quantity_indices
                ):
            ...
            # Restrict fine data
                ...
                if subsample:
                    ...
                    for q, dataset, index in zip(
                        ...
                        data[q][kl_d:ku_d, jl_d:ju_d, il_d:iu_d] = f[dataset][
                            index,
                            block_num,
                            kl_s + o3 : ku_s : s,
                            jl_s + o2 : ju_s : s,
                ...
                # Apply fast (uniform Cartesian) restriction
                    ...
                    io_vals = list(range(s)) if nx1 > 1 else (0,)
                    jo_vals = list(range(s)) if nx2 > 1 else (0,)
                    ko_vals = list(range(s)) if nx3 > 1 else (0,)
                    ...
                    for q, dataset, index in zip(
                        ...
                        for ko in ko_vals:
                            for jo in jo_vals:
                                for io in io_vals:
                                    data[q][kl_d:ku_d, jl_d:ju_d, il_d:iu_d] += f[
                                        dataset
                                    ][
                                        index,
                                        block_num,
                ...
                # Apply exact (volume-weighted) restriction
                    ...
                    i_s_vals = list(range(il_s, iu_s))
                    j_s_vals = list(range(jl_s, ju_s))
                    k_s_vals = list(range(kl_s, ku_s))
                    i_d_vals = list(range(il_d, iu_d))
                    j_d_vals = list(range(jl_d, ju_d))
                    k_d_vals = list(range(kl_d, ku_d))
                    if nx1 > 1:
    ...
    for level in range(max_level):
        ...
        if nx3 > 1:
            vals_level = np.reshape(
                vals * vols,
                (nx3 / stride, stride, nx2 / stride, stride, nx1 / stride, stride),
            )
...
def athinput(filename):
    ...
    with open(filename, "r") as athinput:
        ...
        lines = [
            _f for _f in [i.split("#")[0].strip() for i in athinput.readlines()] if _f
        ]
    data = {}
    ...
    for block in blocks:
        info = list([_f for _f in block.split("\n") if _f])
        key = info.pop(0)[:-1]  # last character is '>'
        data[key] = dict(list(map(parse_line, info)))
    return data
