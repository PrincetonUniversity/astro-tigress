def calc_IQU(nH, Bx, By, Bz, deltas, los="y"):
    ...
    Inu = Bnu * (1.0 - p0 * (cosgam2 - 2.0 / 3.0)) * dtau
    Qnu = p0 * Bnu * cos2phi * cosgam2 * dtau
    Unu = p0 * Bnu * sin2phi * cosgam2 * dtau

    return Inu.sum(axis=ilos), Qnu.sum(axis=ilos), Unu.sum(axis=ilos)
